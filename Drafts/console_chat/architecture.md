...
Стек: **C# / .NET**, DDD, микросервисы, многопоточность, PostgreSQL (единый экземпляр с логическими схемами).
...
- **PostgreSQL** — **единый физический экземпляр**, но с раздельными логическими схемами:
  схема `users` (владелец — UserService) и схема `chat` (владелец — ChatService). RabbitMQ не используется.

Сервисы общаются напрямую по HTTP (например, ChatService запрашивает профили пользователей у UserService).
Такое решение даёт простоту одного инстанса БД, но сохраняет эффект **database-per-service** на уровне схем.
...
Согласованность внутри сервиса обеспечивается транзакциями БД (Unit of Work). Каждый сервис
работает только со своей схемой (`users` — UserService, `chat` — ChatService), а не с общими таблицами:
это сохраняет границу владения данными между сервисами.
...
`outbox` отсутствует (нет брокера). Один физический PostgreSQL, две логические схемы:
`users` (owned by UserService) и `chat` (owned by ChatService).
...
4. **Один PostgreSQL, раздельные схемы** — сервисы работают с отдельными логическими схемами
   (`users`, `chat`), сохраняя границу владения данными; один физический инстанс упрощает деплой.
